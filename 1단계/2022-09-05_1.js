...
function solution(N, stages) {
    let answer = [];
    // 문제를 시도한 인원의 수(실패율의 분모)
    let totalNum = stages.length
    for (let i =1; i < N+1; i ++) {
        // 현재 스테이지에 있는 인원수(실패한사람으로 간주, 실패율의 분자)
        let stageNum = stages.filter(value => value === i).length
        let failRatio = 0
        if (stageNum === 0) {
            failRatio = 0
        } else {
        ...
        totalNum -= stageNum
        answer.push({stage : i, failratio : failRatio})
